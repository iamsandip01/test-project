...
/**
 * Returns the Tailwind CSS class for the station's status color.
 * @param {string} status - The status value of the station.
 * @returns {string} Tailwind CSS class.
 */
const getStatusClass = (status: string): string => {
  const statusObj = STATION_STATUS.find(s => s.value === status);
  return statusObj?.color || 'bg-neutral-500'; // Default to neutral if status not found
};

/**
 * Returns the human-readable label for the station's status.
 * @param {string} status - The status value of the station.
 * @returns {string} Human-readable status label.
 */
const getStatusLabel = (status: string): string => {
  const statusObj = STATION_STATUS.find(s => s.value === status);
  return statusObj?.label || 'Unknown'; // Default to 'Unknown' if status not found
};
</script>
